...
title Uninstall Crazy Kix
cd /d "%~dp0"

echo ========================================
echo Uninstall Crazy Kix
echo ========================================
...
echo Using: "!WDAPP!"
echo.
echo Looking up installed Crazy Kix packages...
set "PFN="
for /f "usebackq tokens=* delims=" %%A in (`"!WDAPP!" list 2^>nul`) do (
  echo %%A | findstr /i /c:"MazenX.CrazyKix_" >nul
  if not errorlevel 1 (
...
if not defined PFN (
  echo.
  echo Crazy Kix is not registered with wdapp right now.
...
echo.
set /p CONFIRM=Uninstall Crazy Kix? [Y/n] 
...
echo Crazy Kix was uninstalled.
