# Live progress wrapper for CrazyKix Win64 MSIXVC packaging.
param(
...
$Host.UI.RawUI.WindowTitle = "CrazyKix Win64 MSIXVC package"

$UeRoot = "F:\Unreal Engine\UE_5.8"
$Project = "F:\Unreal Engine\Projects\CrazyKix\CrazyKix.uproject"
$Archive = "F:\Unreal Engine\Projects\CrazyKix\Packages\Win64_GDK"
$Log = Join-Path $env:TEMP "CrazyKix_Package_Win64_GDK.log"
$RunUat = Join-Path $UeRoot "Engine\Build\BatchFiles\RunUAT.bat"
$MsixOut = "F:\Unreal Engine\Projects\CrazyKix\Saved\Packages\Windows\MSGameStore\Shipping"
...
Write-Host "========================================"
Write-Host "CrazyKix - Package Win64 MSIXVC"
Write-Host "========================================"
...
$wrap = Join-Path $env:TEMP "CrazyKix_Package_Win64_GDK_run.cmd"
$argLine = @(
    "BuildCookRun",
    "-project=`"$Project`"",
    "-platform=Win64",
...
try {
    while (-not $proc.HasExited) {
        ...
        # Keep the bar moving during long silent cook stretches.
        ...
        Write-Progress -Activity "Packaging CrazyKix for Microsoft Store (.msixvc)" -Status $status -PercentComplete $pct -CurrentOperation $detail
    ...
    Write-Host ""
    Write-Progress -Activity "Packaging CrazyKix for Microsoft Store (.msixvc)" -Completed
}
