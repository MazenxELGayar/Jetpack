# Install CrazyKix .msixvc via wdapp. Asks where to put the game.
param(
    [string]$PackagePath,
...
function Read-Folder([string]$Prompt) {
    Write-Host $Prompt
    Write-Host "Example: F:\Games\CrazyKix"
...
Write-Host "========================================"
Write-Host "CrazyKix - GDK local install"
Write-Host "========================================"
Write-Host "wdapp: $wdapp"
Write-Host ""

if (-not $PackagePath) {
    if (Test-Path -LiteralPath $DefaultPackage) { $PackagePath = $DefaultPackage }
    elseif (Test-Path -LiteralPath $ArchivePackage) { $PackagePath = $ArchivePackage }
}
...
if ($drive.Length -lt 2) {
    Write-Host "ERROR: install folder must be a local drive path (for example F:\Games\CrazyKix)." -ForegroundColor Red
    Read-Host "Press Enter to close" | Out-Null
    exit 1
}
...
Write-Host ""
Write-Host "Applying folder / shortcut icons from convertico-Icon.ico ..."
